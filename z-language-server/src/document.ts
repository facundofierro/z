...
class GetErrRequest {
    ...
    private readonly _token: lsp.CancellationTokenSource =
        new lsp.CancellationTokenSource();
...
export class LspDocument {
    ...
    applyEdit(
        ...
        if (lsp.TextDocumentContentChangeEvent.isIncremental(change)) {
            ...
            newContent =
                content.substr(0, start) + change.text + content.substr(end);
        }
...
export class LspDocuments {
    ...
    constructor(
        ...
        const pathNormalizer = (path: URI) =>
            this.client.toTsFilePath(path.toString());
    ...
    private shouldValidate(buffer: LspDocument): boolean {
        // Only validate Z files
        return buffer.languageId === languageModeIds.z;
    }
}
