...
import {
    ...
    ScriptTarget,
    type ts,
    type TypeScriptInitializationOptions,
} from "../ts-protocol.js";
...
export default class FileConfigurationManager {
    public tsPreferences: Required<ts.server.protocol.UserPreferences> =
        deepmerge({}, DEFAULT_TSSERVER_PREFERENCES);
    public workspaceConfiguration: WorkspaceConfiguration = deepmerge(
        {},
    ...
    public setGlobalConfiguration(
        ...
        hostInfo?: TypeScriptInitializationOptions["hostInfo"]
    ): void {
        ...
        const task = (async () => {
            try {
                const response = await this.client.execute(
                    ...
                    { file: document.filepath, ...currentOptions },
                    token
                );
    ...
    private getFormatOptions(
        ...
        const workspacePreferences =
            this.getWorkspacePreferencesForFile(document);
    ...
    public getWorkspacePreferencesForFile(
        document: LspDocument
    ): WorkspaceConfigurationLanguageOptions {
        // Z Language Server only supports Z files - always use Z configuration
        return this.workspaceConfiguration.typescript || {};
    }
    ...
    public getPreferences(
        ...
        const preferences = Object.assign<
            ...
            ts.server.protocol.UserPreferences,
            ts.server.protocol.UserPreferences
        >({}, this.tsPreferences, workspacePreferences?.inlayHints || {});
    ...
    private getAutoImportFileExcludePatternsPreference(
        ...
        return this.tsPreferences.autoImportFileExcludePatterns.map((p) => {
            ...
            return path.posix.isAbsolute(p)
                ...
                ? `/${slashNormalized}`
                : isRelative
                ? path.posix.join(workspaceFolder, p)
                : `/**/${slashNormalized}`;
        });
    }
}
