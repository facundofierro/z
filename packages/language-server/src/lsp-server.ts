...
} from "./features/code-lens/baseCodeLensProvider.js";
...
export class LspServer {
    ...
    private cachedNavTreeResponse =
        new CachedResponse<ts.server.protocol.NavTreeResponse>();
    private implementationsCodeLensProvider: TypeScriptImplementationsCodeLensProvider | null =
        null;
    private referencesCodeLensProvider: TypeScriptReferencesCodeLensProvider | null =
        null;
    ...
    async waitForDiagnosticsForFile(uri: lsp.DocumentUri): Promise<void> {
        ...
        await this.diagnosticQueue.waitForDiagnosticsForTesting(
            document.filepath
        );
    }
    ...
    private async getSymbolLocations(
        ...
        return response.body.map((fileSpan) =>
            toLocation(fileSpan, this.tsClient)
        );
    }
    ...
    async documentSymbol(
        ...
        const response = await this.cachedNavTreeResponse.execute(
            document,
            () =>
    ...
    async hover(
        ...
        const result = await this.tsClient.interruptGetErr(async () => {
            ...
            const response = await this.tsClient.execute(
                ...
                Position.toFileLocationRequestArgs(
                    document.filepath,
                    params.position
                ),
                token
    ...
    async executeCommand(
        ...
        if (
            params.command === Commands.APPLY_WORKSPACE_EDIT &&
            params.arguments
        ) {
            ...
            const [pluginName, configuration] = params.arguments as [
                string,
                unknown
            ];
            ...
            const [_, codeActions] = params.arguments as [
                string,
            ...
            const [uri, position] = (params.arguments || []) as [
                lsp.DocumentUri?,
                lsp.Position?
            ];
            const reporter =
    ...
    protected async applyFileCodeEdits(
        ...
        for (const edit of edits) {
            changes[this.tsClient.toResourceUri(edit.fileName)] =
    ...
    async codeLens(
        ...
        return [
            ...
            ...(await this.referencesCodeLensProvider.provideCodeLenses(
                doc,
                token
            )),
        ];
